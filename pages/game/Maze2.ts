export default class Maze2 {
    ...
    constructor(canvasId: string = 'canvas', size: number = 500) {
        ...
        for (let i = 1; i < this.colors.length; i++) this.colors[i] = [Math.round(i * 2.5), 205 - i * 2, 200]
        ...
        if (!this.running) {
            ...
            if (this.dx > 2)
                for (let y = this.dy; y < this.height; y += this.dy)
                    for (let x = this.dx; x < this.width; x += this.dx) imgArr[(y * this.width + x) * 4 + 3] = this.dot
            this.maxWalk = 0
    ...
    private hidePath(): void {
        ...
        for (let i = 1; i <= this._size_2; i++)
            for (let j = 1; j <= this._size_2; j++) if (this.map[i][j] == Maze2.WAY) this.paintArea(imgArr, i, j, this.bgColor)
    ...
    private paintMaze(): void {
        ...
        if (this.startArea != undefined) this.paintArea(imgArr, this.startArea.i, this.startArea.j, this.startColor)
    ...
    private paintPath(): void {
        ...
        this.teams.forEach((runner) => {
            if (runner.isActive()) this.paintArea(imgArr, runner.getLocation().i, runner.getLocation().j, this.runnerColor)
            else this.paintArea(imgArr, runner.getLocation().i, runner.getLocation().j, this.endColor)
        })
        ...
        if (this.finishArea != undefined) this.paintArea(imgArr, this.finishArea.i, this.finishArea.j, this.finishColor)
    ...
    private createPath(point: Coordinate): Coordinate[] {
        ...
        while (i != this.startArea.i || j != this.startArea.j) {
            ...
            else if (i < this._size_2 && this.map[i + 1][j] > 0 && w - this.map[i + 1][j] > 0) i++
            else if (j < this._size_2 && this.map[i][j + 1] > 0 && w - this.map[i][j + 1] > 0) j++
            else {

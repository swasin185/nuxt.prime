<template>
    <div id="toolbar" style="background: white; width: 180px; float: left; padding: 10px 10px; border: 2px solid; margin: 20px">
        ...
        <input type="number" id="delayInput" min="0" max="100" step="1" value="5" style="width: 50px; float: right; text-align: right" />
        <input type="range" id="delaySlider" min="0" max="100" step="1" value="5" @input="delayInput.value = delaySlider.value" />
        <br />
        ...
        <br /> -->
        <input id="start" type="button" class="button" value="Start" @click="maze.solveMaze(Number(delayInput.value), waiting.checked)" />
        <br />
    </div>
    <canvas id="maze2" width="660" height="660" style="background: #800402; color: white; margin-top: 20px" @click="clickXY"></canvas>
</template>
